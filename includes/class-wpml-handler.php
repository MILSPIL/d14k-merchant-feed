...
class D14K_WPML_Handler
{
    ...
    /**
     * Get the original (default language) term ID for a product_cat term.
     * Used by Custom Labels to resolve category label mappings across WPML languages.
     */
    public function get_original_term_id($term_id)
    {
        if (!$this->is_active()) {
            return $term_id;
        }
        $original_id = apply_filters('wpml_original_element_id', null, $term_id, 'tax_product_cat');
        return $original_id ? (int) $original_id : $term_id;
    }
